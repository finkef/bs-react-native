...
let getItem = (key, ~callback=?, ()) =>
  ...
  | Some(cb) =>
    Js.Promise.then_(to_opt, _getItem(key, Js.Undefined.return(map_cb(cb))))
  };
...
let getAllKeys = (~callback=?, ()) =>
  ...
  | Some(cb) =>
    Js.Promise.then_(to_opt, _getAllKeys(Js.Undefined.return(map_cb(cb))))
  };
...
let multiGet = (keys, ~callback=?, ()) =>
  ...
  | Some(cb) =>
    Js.Promise.then_(to_opt, _multiGet(keys, Js.Undefined.return(map_cb(cb))))
  };
